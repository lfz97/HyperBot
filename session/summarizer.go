...
const (
	...
	// EventThreshold 是自上次摘要后新增事件数的触发阈值。
...
func NewSummarizer(m config.Model) summary.SessionSummarizer {
	...
	sum := summary.NewSummarizer(
		...
		summary.WithChecksAnyContext( // 任一条件满足即触发；混合普通 Checker 与 ContextChecker
			asContextChecker(summary.CheckEventThreshold(EventThreshold)),   // 自上次摘要后新增 N 个事件
			asContextChecker(summary.CheckTimeThreshold(IdleTimeThreshold)), // 自上次摘要后空闲 N 分钟
			summary.CheckContextThreshold( // 自上次摘要后新增 token 占当前模型上下文窗口的比例
