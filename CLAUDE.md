...
**Agent Creation Flow**: `GetEngineService()` runs `preCheckLoad()`: redirects framework logs to `hyperbot.log`, loads/creates `hyperbot.yaml` and `skills/` folder, replaces `{{DATE}}`, `{{OSTYPE}}`, `{{HOME}}`, `{{CWD}}` and other placeholders in system prompt, inits memory/session services, calls `loadSkills()` (populates `helpItems` for HelpTable), then `newRunner()` registers an agent factory that builds the LLMAgent based on `APIType`. All init progress/error messages go to `AgentMessage`.
